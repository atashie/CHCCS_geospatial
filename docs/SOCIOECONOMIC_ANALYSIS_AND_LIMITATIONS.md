...
For a visual, step-by-step walkthrough of the methodology, see the **[Socioeconomic Methodology Walkthrough](../assets/maps/socioeconomic_methodology.html)** — an interactive scrollytelling page generated by `src/socioeconomic_story.py`.
...
**Resolved:** `intersect_zones_with_blockgroups()` now accepts an optional `parcels` parameter and computes dasymetric weights using improved residential parcel area (`weight = frag_res_area / bg_res_area`). Falls back to plain area weighting only for block groups with no residential parcels (5 of 184 fragments in the CHCCS district). For example, one Title I school zone shifted from 19.2% to 23.5% poverty (+4.3pp closer to the known 30-36% FRL rate). As of March 2026, the scrollytelling scripts (`socioeconomic_story.py` and `chccs_demographics_story.py`) read nearest-drive zone demographics from `data/processed/census_dot_zone_demographics.csv` — a dot-level aggregation exported by `school_socioeconomic_analysis.py`. As of April 2026, that dot-level aggregation is universe-correct: extensive-percentage metrics (`pct_below_185_poverty`, `pct_zero_vehicle`, `pct_renter`) are computed by per-dot attribution of block-level numerator/denominator pairs (`Σ num / Σ den × 100`) rather than a population-weighted mean of block percentages, which previously over- or under-counted magnet-zone statistics by up to 25 pp where group-quarters population or household-size distributions diverged from total population (see `docs/IMPLEMENTATION_NOTES.md` → "Zone-Level Percentage Metrics — Universe-Mismatch Bug"). Population-denominated percentages (`pct_minority`, `pct_elementary_age`, `pct_young_children`) continue to use the dot-weighted mean, which is provably correct for them.
...
**Implemented.** `intersect_zones_with_blockgroups()` now accepts residential parcel data and computes weights as `fragment_residential_area / bg_residential_area` using spatial index lookups. Falls back to plain area weighting for block groups with no residential parcels. Result: one Title I school zone shifted from 19.2% to 23.5% poverty (+4.3pp closer to known FRL), and population estimates changed significantly across all zones. Additionally, `school_socioeconomic_analysis.py` now exports `census_dot_zone_demographics.csv` — a per-zone aggregation for all 5 zone types that mirrors the interactive map's JS computation. As of April 2026 the aggregation is universe-correct: population-denominated percentages (`pct_minority`, `pct_elementary_age`, `pct_young_children`) use a dot-weighted mean of block values; extensive-denominator percentages (`pct_below_185_poverty`, `pct_zero_vehicle`, `pct_renter`) use per-dot attribution of block raw numerators and denominators (Σ num / Σ den). The scrollytelling scripts read this CSV for drive-zone demographics, ensuring consistency with the interactive map.
...
Geocoded sales are spatially joined to all zone types (attendance zones, walk zones, nearest walk/bike/drive) and Census blocks. Per-zone and per-block summaries include:
