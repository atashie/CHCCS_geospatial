...
**Implemented.** `intersect_zones_with_blockgroups()` now accepts residential parcel data and computes weights as `fragment_residential_area / bg_residential_area` using spatial index lookups. Falls back to plain area weighting for block groups with no residential parcels. Result: one Title I school zone shifted from 19.2% to 23.5% poverty (+4.3pp closer to known FRL), and population estimates changed significantly across all zones. Additionally, `school_socioeconomic_analysis.py` now exports `census_dot_zone_demographics.csv` — a dot-level aggregation for all 5 zone types that matches the interactive map's JS computation. The scrollytelling scripts read this CSV for drive-zone demographics, ensuring exact consistency with the interactive map.
