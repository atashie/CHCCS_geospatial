...
**Resolved:** `intersect_zones_with_blockgroups()` now accepts an optional `parcels` parameter and computes dasymetric weights using improved residential parcel area (`weight = frag_res_area / bg_res_area`). Falls back to plain area weighting only for block groups with no residential parcels (5 of 184 fragments in the CHCCS district). For example, one Title I school zone shifted from 19.2% to 23.5% poverty (+4.3pp closer to the known 30-36% FRL rate). As of March 2026, all callers — including the scrollytelling scripts (`socioeconomic_story.py` and `chccs_demographics_story.py`) for nearest-drive zone demographics — pass residential parcel data, ensuring consistent dasymetric weighting across all zone types.
...
**Implemented.** `intersect_zones_with_blockgroups()` now accepts residential parcel data and computes weights as `fragment_residential_area / bg_residential_area` using spatial index lookups. Falls back to plain area weighting for block groups with no residential parcels. Result: one Title I school zone shifted from 19.2% to 23.5% poverty (+4.3pp closer to known FRL), and population estimates changed significantly across all zones. All callers now pass parcel data, including the scrollytelling scripts for nearest-drive zone demographics.
